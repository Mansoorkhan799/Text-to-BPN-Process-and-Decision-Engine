...
const Records: React.FC<RecordsProps> = ({ user }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const recordsWithDates = data.map((record: any) => ({
          ...
          // Normalize parentId: convert empty string, undefined, or null to null
          parentId: (record.parentId && typeof record.parentId === 'string' && record.parentId.trim() !== '') ? record.parentId : null,
  ...
  const filteredRecords = useMemo(() => {
    ...
    const filtered = records.filter(record => {
      // Search filter - check if search term matches title or tag
      const matchesSearch = !searchTerm || 
        (record.title && record.title.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (record.tag && record.tag.toLowerCase().includes(searchTerm.toLowerCase()));
      
      // Tag filter - if 'all' is selected, show all; otherwise match the tag
      const matchesTag = filterTag === 'all' || (record.tag && record.tag === filterTag);
      
      // Owner filter - if 'all' is selected, show all; otherwise match the owner
      const matchesOwner = filterOwner === 'all' || (record.owner && record.owner === filterOwner);
      
      const passes = matchesSearch && matchesTag && matchesOwner;
      
      if (!passes && records.length > 0) {
    ...
    // Start with only parent records (records with no parentId - null, undefined, or empty string)
    // A record is a parent if parentId is null/undefined/empty, regardless of level
    const parentRecords = filtered.filter(record => {
      // Check if parentId is falsy (null, undefined, empty string, or not set)
      const isParent = !record.parentId;
      return isParent;
    });
    
    // If no parent records found, show all filtered records (they might all be top-level)
    // This handles the case where all records are actually top-level but parentId wasn't set correctly
    const recordsToInclude = parentRecords.length > 0 ? [...parentRecords] : [...filtered];
    const allVisibleRecords = new Set(recordsToInclude.map(r => r.id));
    ...
    console.log('🔍 Filtering results:', {
      ...
      sampleRecord: records[0] ? {
        ...
        parentIdType: typeof records[0].parentId,
        level: records[0].level
      } : null
    });
    
    // Debug: Log all records to see their parentId values
    console.log('🔍 All records parentId values:', records.map(r => ({
      id: r.id,
      title: r.title,
      parentId: r.parentId,
      parentIdType: typeof r.parentId,
      isParent: !r.parentId || r.parentId === null || r.parentId === ''
    })));
