...
// Create transporter function (create it dynamically to ensure env vars are loaded)
...
export const sendOTPEmail = async (email: string, otp: string) => {
  ...
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
    console.error('Email credentials not configured. EMAIL_USER and EMAIL_PASSWORD must be set in .env.local');
    return false;
  }
  ...
  try {
    ...
    // Send email directly (skip verify for faster execution)
    ...
    if (error.code === 'EAUTH' || error.responseCode === 535) {
      ...
      console.error('3. Generate a new App Password from: https://myaccount.google.com/apppasswords');
      console.error('4. Update EMAIL_PASSWORD in .env.local with the 16-character App Password');
    }
